...
//            Toast.makeText(mContext, "微信对接中,请使用支付宝支付", Toast.LENGTH_SHORT).show();
            ...
            if (type == 1) {
                ...
                Map map = MapUtil.getInstance().addParms("totalAmount", submitOrderBean.getTotalAmount()).addParms("masterNo", submitOrderBean.getMasterNo()).addParms("productName", "易购商城").addParms("map", jsonString).addParms("type", 1).build();
...
//                WaitDialog.show((AppCompatActivity) mContext, null);
                ...
                Map map = MapUtil.getInstance().addParms("totalAmount", submitOrderBean.getTotalAmount()).addParms("masterNo", submitOrderBean.getMasterNo()).addParms("productName", "易购商城").addParms("userCode", SPUtil.getUserCode()).addParms("type", 0).build();
                ProcessDialogUtil.showProcessDialog(mContext);
//                WaitDialog.show((AppCompatActivity) mContext, null);
