...
public class OrderConfirmActivity extends BaseActivity<OrderConfirmView, OrderConfirmPresneter> implements OrderConfirmView {
    ...
    public void initData() {
        ...
        if (SPUtil.getStringValue(CommonResource.LEVELID).equals("2")){
            orderConfirmPrice.setText(confirmBean.getVipPrice() + "");
            orderConfirmTotalPrice.setText("￥" + ArithUtil.mul(confirmBean.getVipPrice(), confirmBean.getQuantity()));
        }else {
    ...
    public void loadPostage(PostageBean postageBean) {
        totalMoney = ArithUtil.sub(postageBean.getTotal(), postageBean.getFeight());
