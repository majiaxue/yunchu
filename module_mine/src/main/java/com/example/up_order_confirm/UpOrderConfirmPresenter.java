...
//            WaitDialog.show((AppCompatActivity)mContext,null);
            if (isWeChat) {
                ...
                Observable observable = RetrofitUtil.getInstance().getApi(CommonResource.BASEURL_9004).postHeadWithBody(CommonResource.LIBAO_WXPAY + "?totalAmount=" + bean.getPrice() + "&userCode=" + SPUtil.getUserCode() + "&levelId=" + levelId + "&productName=易购商城-" + name + "&type=1", requestBody, SPUtil.getToken());
